...
type QuizContentLinkInput = z.infer<typeof QuizContentLinkInputSchema>;
...
function normalizeQuizContentLinks(contentLinks: QuizContentLinkInput[]) {
  const unique = new Map<string, QuizContentLinkInput>();
...
async function validateQuizContentLinks(
  userId: string,
  role: string,
  contentLinks: QuizContentLinkInput[]
): Promise<LinkValidationResult> {
  for (const link of contentLinks) {
    let item: { teacherId: string } | null = null;
    let label = "";
...
function formatQuiz(item: {
  ...
  contentLinks: Array<{
    quizId: string;
    linkedType: "grammar" | "story" | "vocab" | "pronunciation" | "quiz";
    linkedId: string;
  }>;
  questions: Array<{
    id: string;
...
async function loadFormattedQuiz(quizId: string) {
...
quizzesRouter.post("/", authRequired, teacherOrAdmin, async (req, res, next) => {
  try {
    ...
    const linkValidation = await validateQuizContentLinks(
      req.auth!.sub,
      req.auth!.role,
      contentLinks
    );
...
quizzesRouter.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const nextContentLinks =
        input.contentLinks === undefined
          ? undefined
...
quizzesRouter.post("/attempts", authRequired, async (req, res, next) => {
  try {  // Validate submitted quiz attempt.
...
//This code calculates the score and saves the quiz attempt and 
// answers inside one database transaction.
    const scoring = scoreQuizAttempt(
...
// Update final score after saving answers.
