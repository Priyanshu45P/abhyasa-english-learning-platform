...
//This code creates a unique six-character classroom code for each new classroom.
function generateCode(length = 6) {
  const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
  let code = "";
// Randomly pick six characters.
...
// Try 10 times to avoid duplicate code.
...
type ClassroomContentType = z.infer<typeof ClassroomContentTypeSchema>;
...
async function getAccessibleClassroom(
  ...
  if (!classroom) {
    return { error: { status: 404, message: "Classroom not found" } as const };
  }

  const isTeacherOwner = classroom.teacherId === userId;
  const isJoinedStudent = classroom.students.some(
...
async function getManageableClassroom(
  classroomId: string,
  userId: string,
  role: string
) {
...
async function getClassroomContentPayload(classroomId: string) {
  const classroom = await prisma.classroom.findUnique({
    where: { id: classroomId },
    select: {
      id: true,
...
async function ensureTeacherOwnsContent(
  teacherId: string,
  ...
  switch (contentType) {
    case "grammar": {
      ...
      if (!item) {
        return {
          error: { status: 404, message: "Grammar lesson not found" } as const,
        };
      }
    ...
    case "story": {
      ...
      if (!item) {
        return {
          error: { status: 404, message: "Story not found" } as const,
        };
      }
    ...
    case "vocab": {
      ...
      if (!item) {
        return {
          error: { status: 404, message: "Vocabulary list not found" } as const,
        };
      }
    ...
    case "quiz": {
      ...
      if (!item) {
        return {
          error: { status: 404, message: "Quiz not found" } as const,
        };
      }
...
//This code assigns grammar, story, vocabulary, pronunciation,
//  or quiz content to a classroom without creating duplicates.
async function assignContentToClassroom(
  classroomId: string,
  contentType: ClassroomContentType,
  contentId: string
) {
...
async function removeContentFromClassroom(
  classroomId: string,
  contentType: ClassroomContentType,
  contentId: string
) {
...
classroomsRouter.get("/:id", authRequired, async (req, res, next) => {
  try {
    const result = await getAccessibleClassroom(
      req.params.id,
      req.auth!.sub,
      req.auth!.role
    );
...
classroomsRouter.get("/:id/content", authRequired, async (req, res, next) => {
  try {
    const access = await getAccessibleClassroom(
      req.params.id,
      req.auth!.sub,
      req.auth!.role
    );
...
//This code lets a teacher or admin create a classroom and saves its unique joining code.
...
//This code allows only students to join a valid classroom and prevents duplicate joining using upsert.
classroomsRouter.post("/join", authRequired, async (req, res, next) => {
  try { // Only students can join classrooms.
    if (req.auth!.role !== "student") {
...
// upsert prevents same student joining same classroom twice.
...
classroomsRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const classroomResult = await getManageableClassroom(
        req.params.id,
        req.auth!.sub,
        req.auth!.role
      );

      const classroomResultError =
        "error" in classroomResult ? classroomResult.error : undefined;
      ...
      if (req.auth!.role !== "admin") {
        const contentOwnership = await ensureTeacherOwnsContent(
          req.auth!.sub,
          input.contentType,
          input.contentId
        );

        const contentOwnershipError =
          "error" in contentOwnership ? contentOwnership.error : undefined;
      ...
      await assignContentToClassroom(
        req.params.id,
        input.contentType,
        input.contentId
      );
...
classroomsRouter.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      await removeContentFromClassroom(
        req.params.id,
        input.contentType,
        input.contentId
      );
