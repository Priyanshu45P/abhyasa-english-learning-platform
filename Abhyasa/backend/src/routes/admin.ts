...
adminRouter.get("/dashboard", async (_req, res, next) => {
  try { // Count all important platform records in parallel.
    const [
      usersCount,
      teachersCount,
      studentsCount,
...
adminRouter.get("/users", async (req, res, next) => {
  try {
    ...
    const parsedRole =
      role && ["admin", "teacher", "student"].includes(role) ? role : "";

    const users = await prisma.user.findMany({
      where: {
        ...(parsedRole ? { role: parsedRole as "admin" | "teacher" | "student" } : {}),
        ...(search
          ? {
              OR: [
                {
...
adminRouter.get("/teacher-analytics", async (_req, res, next) => {
  try { // Find all teachers and related data.
    ...
    return res.json(
      teachers.map((teacher) => { // Get all attempts from teacher-created quizzes.
...
// Calculate average quiz score.
 ...
 // Count unique students across all teacher classrooms.
        const uniqueStudentIds = new Set(
          teacher.taughtClassrooms.flatMap((classroom) =>
            classroom.students.map((student) => student.studentId)
          )
        );
        ...
        return {
          ...
          classrooms: teacher.taughtClassrooms.map((classroom) => ({
            ...
            assignedContentCount:
              classroom.grammarAssignments.length +
...
adminRouter.get("/student-analytics", async (_req, res, next) => {
  try { // Find all students and their activity.
    const students = await prisma.user.findMany({
      ...
      select: {
        ...
        pronunciationAttempts: { // Pronunciation attempts
          select: {
            id: true,
            exerciseId: true,
            score: true,
    ...
    return res.json( // Count completions by content type.
      students.map((student) => {
        const completedByType = student.progressCompletions.reduce(
          (acc, item) => {
          ...
          },
          {
            ...
            pronunciation: 0,
            quiz: 0,
          } as Record<"grammar" | "story" | "vocab" | "pronunciation" | "quiz", number>
        );
 // Average quiz percentage.
        const averageQuizScore =
          student.quizAttempts.length === 0
            ? 0
            : Math.round(
                student.quizAttempts.reduce((sum, attempt) => {
                  if (attempt.totalQuestions === 0) return sum;
                  return sum + (attempt.score / attempt.totalQuestions) * 100;
                }, 0) / student.quizAttempts.length
              );
 // Average Pronunciation percentage.
        const averagePronunciationScore =
          student.pronunciationAttempts.length === 0
            ? 0
            : Math.round(
                student.pronunciationAttempts.reduce(
                  (sum, attempt) => sum + Number(attempt.matchPercent),
                  0
                ) / student.pronunciationAttempts.length
              );
