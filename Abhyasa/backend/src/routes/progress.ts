...
// Find classrooms owned by teacher.
...
// Check if selected student belongs to teacher's classroom.
      const isStudentInTeacherClass = classrooms.some((classroom) =>
...
// Collect unique student IDs from teacher's classrooms.
      const studentIds = Array.from(
        new Set(
          classrooms.flatMap((classroom) =>
            classroom.students.map((row) => row.studentId)
          )
        )
      );
      ...
      const quizIds = Array.from(
        new Set(quizAttempts.map((attempt) => attempt.quizId))
      );

      const classroomQuizLinks =
        quizIds.length === 0
          ? []
