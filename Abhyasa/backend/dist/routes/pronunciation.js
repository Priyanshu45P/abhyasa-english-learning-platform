...
pronunciationRouter.get("/attempts", authRequired, async (req, res, next) => {
    try {
        const exerciseId = typeof req.query.exerciseId === "string" ? req.query.exerciseId : "";
        if (!exerciseId.trim()) {
...
pronunciationRouter.post("/", authRequired, teacherOrAdmin, async (req, res, next) => {
    try {
...
pronunciationRouter.patch("/:id", authRequired, teacherOrAdmin, async (req, res, next) => {
    try {
...
pronunciationRouter.delete("/:id", authRequired, teacherOrAdmin, async (req, res, next) => {
    try {
        const existing = await prisma.pronunciationExercise.findUnique({
            where: { id: req.params.id },
            select: {
        ...
        if (existing._count.attempts > 0) {
            return res.status(400).json({
                error: "Pronunciation exercise cannot be deleted after students have attempted it",
            });
        }
