...
export function getWordSimilarityPercent(expected, transcript) {
...
export function getSentenceSimilarityPercent(expected, transcript) {
    ...
    for (let i = 0; i < Math.min(expectedWords.length, transcriptWords.length); i += 1) {
        if (expectedWords[i] === transcriptWords[i]) {
            ...
            const wordSimilarity = getWordSimilarityPercent(expectedWords[i], transcriptWords[i]);
            if (wordSimilarity >= 75)
                matched += 1;
        }
    }
...
export function buildPronunciationResult(expected, transcript, type) {
    ...
    if (!normalizedExpected || !normalizedTranscript) {
        ...
        matchPercent = getWordSimilarityPercent(normalizedExpected, normalizedTranscript);
    }
    else {
        const sentencePercent = getSentenceSimilarityPercent(normalizedExpected, normalizedTranscript);
        const charPercent = getWordSimilarityPercent(normalizedExpected, normalizedTranscript);
