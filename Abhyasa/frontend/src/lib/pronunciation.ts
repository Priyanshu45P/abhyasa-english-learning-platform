...
export function getSentenceSimilarityPercent(expected: string, transcript: string) {
  ...
  for (
    let i = 0;
    i < Math.min(expectedWords.length, transcriptWords.length);
    i += 1
  ) {
...
// Compares the expected text with the student's speech transcript and returns a score with feedback.
export function buildPronunciationResult(
  expected: string,
  transcript: string,
  type: "word" | "phrase" | "sentence"
): PronunciationResult { // Clean both texts before comparison.
  ...
  if (!normalizedExpected || !normalizedTranscript) {
    ...
    matchPercent = getWordSimilarityPercent( // Single word: compare character similarity.
      normalizedExpected,
      normalizedTranscript
    );
  } else { // Phrase/sentence: compare word match and character similarity.
  ...
  if (matchPercent >= 90) {
    ...
    score = "close";
    feedback =
      "Good attempt. You are close, but a few sounds need clearer pronunciation.";
  } else if (matchPercent >= 45) {
    score = "mismatch";
    feedback =
      "You are partly correct. Listen again, speak slowly, and try one more time.";
  }
